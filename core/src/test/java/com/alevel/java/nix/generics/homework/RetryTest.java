...
import com.alevel.java.nix.exceptions.homework.Block;
import com.alevel.java.nix.exceptions.homework.Retry;
...
class RetryTest{
    ...
    void shouldFailWhenAttemptsRunOut() {
        var retry = Retry.builder(new RetryTest.FailingBlock(3)).times(3).build();
        var failure = assertThrows(RetryTest.FailingBlockException.class, retry::run);
        assertEquals(0, failure.remainingFailures);

        retry = Retry.builder(new RetryTest.FailingBlock(4)).times(3).build();
        failure = assertThrows(RetryTest.FailingBlockException.class, retry::run);
        assertEquals(1, failure.remainingFailures);
    }

    @Test
    void shouldSucceedIfAttemptIsSuccessful() {
        var retry = Retry.builder(new RetryTest.FailingBlock(2)).times(3).build();
        assertDoesNotThrow(retry::run);

        retry = Retry.builder(new RetryTest.FailingBlock(0)).times(3).build();
        assertDoesNotThrow(retry::run);
    }

    @Test
    void shouldDelayRetriesIncrementally() {
        var retry = Retry.builder(new RetryTest.FailingBlock(2)).times(3).pause(5).build();
        assertDoesNotThrow(retry::run);

        retry = Retry.builder(new RetryTest.DelayedBlock(30)).times(3).pause(10).build();
        assertDoesNotThrow(retry::run);

        retry = Retry.builder(new RetryTest.DelayedBlock(70)).times(3).pause(10).build();
        assertThrows(RetryTest.DelayedBlockException.class, retry::run);
    }
    ...
    private static final class FailingBlock implements Block {
        ...
        public void run() throws Exception {
            if (--failures >= 0) {
                throw new RetryTest.FailingBlockException(failures);
            }
        }
    }
    ...
    private static final class DelayedBlock implements Block {
        ...
        public void run() throws Exception {
            ...
            if (delay < requiredDelay) {
                throw new RetryTest.DelayedBlockException(delay);
            }
        }
    }
